...
// ======= DATA (edita aquí tus propiedades) =======
const PROPS = [
  {
    id: "AM-001",
    titulo: "Casa individual 3D/2B – Villa Alemana",
    operacion: "arriendo", // "venta" | "arriendo"
    comuna: "Villa Alemana",
    region: "Valparaíso",
    precio: 680000, // CLP mensual o precio de venta
    moneda: "CLP",
    dormitorios: 3,
    banos: 2,
    estacionamientos: 1,
    ...
    descripcion:
      "Casa luminosa en pasaje tranquilo. Cocina cerrada, living-comedor, patio y estacionamiento para 1 vehículo.",
    imagenes: [
  ...
  },
  {
    ...
    descripcion:
      "A pasos de comercio, locomoción y servicios. Ideal para primera vivienda o inversión.",
    imagenes: [
  ...
  },
  {
    ...
    descripcion:
      "Terreno plano con excelente conectividad. Ideal para proyecto familiar o inversión.",
    imagenes: [
...
const Input = (props) => (
  <input
    {...props}
    className={`w-full rounded-xl border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-black ${
      props.className || ""
    }`}
  />
);
  ...
  return (
    <div className="group rounded-2xl border border-gray-200 overflow-hidden shadow-sm hover:shadow-md transition bg-white">
      <div className="relative aspect-[4/3] overflow-hidden">
        ...
        {p.destacado && (
          <div className="absolute top-3 left-3 bg-black/80 text-white text-xs px-2 py-1 rounded-full">Destacado</div>
        )}
      </div>
      <div className="p-4 space-y-3">
        <div className="flex items-start justify-between gap-3">
          <h3 className="font-semibold leading-snug">{p.titulo}</h3>
          <Badge className="shrink-0">{p.operacion === "venta" ? "Venta" : "Arriendo"}</Badge>
        </div>
        <div className="text-lg font-bold">{currency(p.precio, p.moneda)}</div>
        ...
        <div className="flex items-center gap-2 pt-1">
          {p.etiquetas?.slice(0, 3).map((e) => (
            <span key={e} className="text-xs text-gray-500">#{e}</span>
          ))}
        </div>
        <div className="pt-2 flex gap-2">
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="inline-flex items-center justify-center rounded-xl border border-gray-300 px-3 py-2 text-sm hover:bg-gray-50"
          >
            WhatsApp
          </a>
          <button
            onClick={() => onOpen(p)}
            className="inline-flex items-center justify-center rounded-xl bg-black text-white px-3 py-2 text-sm hover:opacity-90"
          >
            Ver ficha
          </button>
        </div>
      ...
      <div className="absolute right-0 top-0 h-full w-full max-w-2xl bg-white shadow-xl flex flex-col">
        ...
        <div className="p-4 overflow-y-auto space-y-6">
          ...
          <div className="space-y-2">
            <div className="text-2xl font-bold">{currency(p.precio, p.moneda)} {p.operacion === "venta" ? "• Venta" : "• Arriendo"}</div>
          ...
          <div className="flex gap-2">
            <a
              href={`mailto:${p.contactoEmail}?subject=${encodeURIComponent(
                `[Interés ${p.id}] ${p.titulo}`
...
export default function App() {
  ...
  const [op, setOp] = useState("todas"); // venta | arriendo | todas
  const [comuna, setComuna] = useState("todas");
  const [min, setMin] = useState("");
  const [max, setMax] = useState("");
  const [dorms, setDorms] = useState("todas");
  const [sort, setSort] = useState("recientes"); // precio_asc | precio_desc | recientes
  const [open, setOpen] = useState(false);
  const [selected, setSelected] = useState(null);

  const comunas = useMemo(() => ["todas", ...Array.from(new Set(PROPS.map((p) => p.comuna)))], []);
  ...
  const filtered = useMemo(() => {
    ...
    switch (sort) {
      ...
      case "precio_desc":
        ...
        break;
      default: // recientes: destacados primero y por id (simulado)
        data.sort((a, b) => Number(b.destacado) - Number(a.destacado) || String(b.id).localeCompare(String(a.id)));
    }
    return data;
    ...
    <div className="min-h-screen bg-gray-50 text-gray-900">
      ...
      <nav className="sticky top-0 z-40 bg-white/80 backdrop-blur border-b">
        <div className="max-w-7xl mx-auto flex items-center justify-between px-4 sm:px-6 lg:px-8 h-16">
          <a href="#home" className="font-bold text-lg">Alarcón & Matus Propiedades</a>
          <div className="hidden md:flex items-center gap-6 text-sm">
            <a href="#listados" className="hover:opacity-70">Listados</a>
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="inline-flex items-center justify-center rounded-xl bg-black text-white px-3 py-2 hover:opacity-90"
            >
              WhatsApp
            </a>
          </div>
      ...
      <header id="home" className="relative overflow-hidden">
        <div className="absolute inset-0 -z-10">
          <div className="h-full w-full bg-[radial-gradient(ellipse_at_top_left,_var(--tw-gradient-stops))] from-gray-100 via-white to-gray-100" />
        </div>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          <div className="grid md:grid-cols-2 gap-8 items-center">
            <div className="space-y-6">
              <h1 className="text-3xl md:text-5xl font-extrabold leading-tight">
                Publica, vende y arrienda con <span className="underline decoration-gray-300">confianza</span>
              </h1>
              <p className="text-gray-600 md:text-lg">
                Gestión integral de propiedades en la Región de Valparaíso. Transparencia, velocidad y un servicio cercano.
              </p>
              <div className="flex gap-3">
                <a href="#listados" className="rounded-xl bg-black text-white px-4 py-2 text-sm md:text-base">Ver propiedades</a>
                <a href="#contacto" className="rounded-xl border px-4 py-2 text-sm md:text-base">Publica la tuya</a>
              </div>
              <div className="flex items-center gap-4 pt-2 text-sm text-gray-600">
                <span>✓ Contratos y filtros de inquilinos</span>
                <span>✓ Reportes al propietario</span>
                <span>✓ Fotos y difusión</span>
              </div>
            </div>
            <div className="rounded-3xl overflow-hidden shadow-lg">
              <img src="https://picsum.photos/id/1003/1200/800" alt="Hero" className="w-full h-full object-cover" />
            </div>
          </div>
        </div>
      </header>
      ...
      <Section id="listados" title="Propiedades disponibles" subtitle="Filtra por operación, comuna, precio y dormitorios.">
        <div className="grid grid-cols-2 md:grid-cols-6 gap-3 bg-white p-4 rounded-2xl border">
          ...
          <div className="col-span-2 md:col-span-6">
            <Input
              placeholder="Buscar por título, comuna, etiqueta…"
              value={query}
        ...
        <div className="mt-8 grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {filtered.map((p) => (
            <PropertyCard key={p.id} p={p} onOpen={openFicha} />
          ))}
          {filtered.length === 0 && (
            <div className="col-span-full text-center text-gray-500 py-12 border rounded-2xl bg-white">No se encontraron propiedades con esos filtros.</div>
          )}
        </div>
      </Section>
      ...
      <Section id="servicios" title="Servicios para propietarios" subtitle="Nos encargamos de punta a punta para que no te preocupes de nada.">
        <div className="grid md:grid-cols-3 gap-6">
          {[
            {
              t: "Publicación y Difusión",
              d: "Fotos, video, copywriting y publicación en portales y redes.",
            },
            {
              t: "Selección de Inquilinos",
              d: "Pre-evaluación, verificación de antecedentes y scoring.",
            },
            { t: "Gestión de Contratos", d: "Borradores, firmas y renovación o término." },
          ].map((s) => (
            <div key={s.t} className="rounded-2xl border bg-white p-6 shadow-sm">
        ...
        <div className="grid md:grid-cols-2 gap-8">
          ...
          >
            ...
            <div>
              ...
              <textarea name="mensaje" required placeholder="Quiero publicar mi casa / consultar por AM-001" className="w-full rounded-xl border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-black min-h-[120px]" />
            </div>
            <div className="flex gap-3">
              <button type="submit" className="rounded-xl bg-black text-white px-4 py-2 text-sm">Enviar por WhatsApp</button>
              <a
                href={`mailto:${PROPS[0].contactoEmail}?subject=${encodeURIComponent(
          ...
          <div className="space-y-4">
            <div className="rounded-2xl border bg-white p-6 shadow-sm">
              ...
              <p className="text-sm text-gray-700">Alarcón & Matus Propiedades</p>
      ...
      <footer className="border-t">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 text-sm text-gray-600 flex flex-col md:flex-row gap-3 md:items-center md:justify-between">
          <p>© {new Date().getFullYear()} Alarcón & Matus Propiedades. Todos los derechos reservados.</p>
          <div className="flex gap-4">
            <a href="#home" className="hover:opacity-70">Inicio</a>
            <a href="#listados" className="hover:opacity-70">Propiedades</a>
            <a href="#contacto" className="hover:opacity-70">Contacto</a>
          </div>
        </div>
      </footer>

      <Drawer open={open} onClose={() => setOpen(false)} p={selected} />
    </div>
  );
}
